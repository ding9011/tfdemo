...
tf.app.flags.DEFINE_string('num_dev_utt', int(get("num_dev_utt")), 'the number of dev utt')
...
set_section('retrain')
tf.app.flags.DEFINE_integer('checkpoint', int(get("checkpoint")), 'retrain model store step')
...
def main(_):
    ...
    with graph.as_default(), tf.device('/cpu:0'):
        ...
        with tf.variable_scope(tf.get_variable_scope()):
            with tf.device('/gpu:0'):
                ...
                logits, embeddings1, embeddings2 = prepare_model_affine_feats(inputs, num_speakers, FLAGS)
    ...
    check_point_dir = os.path.join(os.path.dirname(os.path.abspath(model_path)), 'train_logs-' + str(FLAGS.checkpoint))
