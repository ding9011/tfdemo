...
if not set_section("envpath"):
    print("can't find this configuration section")
    exit(1)
os.environ['KALDI_ROOT'] = get('KALDI_ROOT')
...
tf.app.flags.DEFINE_list('gpu_array', get('gpu_array').split(','), 'gpu ids')
...
def __generate_trials(trials, enroll_dict, test_dict):
    with open(trials, 'w') as f:
        for k1,v1 in enroll_dict.items():
...
def _valid_model(_sess, _embedding1, _embedding2, _inputs, _labels, _dev_accuracy_log, _dev_utt, _dev_trials_symbol, _epoch, _step, _FLAGS):
    ...
    for i in range(len(dev_dvectors)):
        if dev_keys[i] in spk_temp_dvectors.keys():
            ...
            spk_temp_dvectors[dev_keys[i]].append(dev_dvectors[i])
    for k,v in spk_temp_dvectors.items():
...
def main(_):
    ...
    gpu_list = []
    if FLAGS.appoint_gpu:
        gpu_list = [ int(i.strip()) for i in FLAGS.gpu_array]
    else:
        gpu_list = [ int(i) for i in range(FLAGS.num_gpu) ]
    ...
    with graph.as_default(), tf.device('/cpu:0'):
        ...
        with tf.variable_scope(tf.get_variable_scope()):
            inputs = tf.split(tf.placeholder(tf.float32, [num_gpu * FLAGS.batch_size, lstm_time, neighbor_dim, FLAGS.feature_dim]), num_gpu, 0)
            labels = tf.split(tf.placeholder(tf.int32, [num_gpu * FLAGS.batch_size]), num_gpu, 0)
            ...
            models = []
            for i in gpu_list:
                with tf.device('/' + device_type + ':' + str(i)), tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
                    with tf.name_scope('GPU_%d' % i):
                        logits, embeddings1, embeddings2 = prepare_model(inputs[i], num_speakers, FLAGS)
                        softmax_result = tf.nn.softmax(logits)
                        label_onehot = tf.one_hot(labels[i] - 1, depth=num_speakers)
                        cross_entropy = tf.nn.softmax_cross_entropy_with_logits_v2(logits=logits, labels=label_onehot, name='cross_entropy')
                        with tf.name_scope('loss'):
                            loss = tf.reduce_mean(cross_entropy)
                            grad = opt.compute_gradients(loss)
                        with tf.name_scope('result_print'):
                            pred = tf.argmax(logits, 1)
                            true_pred = tf.argmax(label_onehot, 1)
                            prob = tf.nn.softmax(logits)
                        with tf.name_scope('accuracy'):
                            correct_prediction = tf.equal(tf.argmax(logits, 1), tf.argmax(label_onehot, 1))
                            accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32), name='accuracy')
                        models.append([pred, true_pred, prob, accuracy, loss, grad])

        print(len(models))
        tower_pred, tower_true_pred, tower_prob, tower_acc, tower_loss, tower_grads = zip(*models)
        preds = tf.concat(tower_pred, 0)
        true_preds =tf.concat(tower_true_pred, 0)
        aver_prob = tf.reduce_mean(tf.reduce_max(tower_prob))
        aver_accuracy = tf.reduce_mean(tower_acc)
        ...
        summaries.add(tf.summary.scalar('eval/Loss', aver_loss_op))
        summaries.add(tf.summary.scalar('accuracy', aver_accuracy))
    ...
    with tf.Session(graph=graph, config=config) as sess:
        ...
        if FLAGS.small_dataset_test:
            ...
            for epoch in range(FLAGS.num_epochs):
                ...
                for batch_num in range(FLAGS.num_small_test_batch_size):
                    start_idx = num_gpu * batch_num * FLAGS.batch_size
                    end_idx = num_gpu * (batch_num + 1) * FLAGS.batch_size
                    batch_datas = np.array(preload_batch_datas[start_idx: end_idx])
                    batch_labels = preload_batch_labels[start_idx: end_idx]
                    _, loss_value, train_accuracy, out_judge, \
                    out_true_pred, out_mean_max_prob, out_learning_rate, step = sess.run(
                        [train_op, aver_loss_op, aver_accuracy, \
                        preds, true_preds, aver_prob, learning_rate, global_step], \
                    ...
                    print("mean max probability result:", out_mean_max_prob)
                    print("program out labels:", out_pred + 1)
                    print("true lables:", out_true_pred + 1)
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            for batch_num in range(num_batches_per_epoch):
                ...
                batch_datas, batch_labels = preload_batch_datas[0:num_gpu * FLAGS.batch_size], preload_batch_labels[0:num_gpu * FLAGS.batch_size]
                print(batch_datas)
                print(batch_labels)
                ...
                preload_batch_datas = preload_batch_datas[num_gpu * FLAGS.batch_size:]
                preload_batch_labels = preload_batch_labels[num_gpu * FLAGS.batch_size:]
